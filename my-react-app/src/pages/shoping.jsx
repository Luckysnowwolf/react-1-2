...
function handleAmountOf() {
    ...
    return (
        <div class="shopingSection">
            ...
            <button class="add" onClick={() => handleAddPoster(index)}>add poster</button><button class="minus" onClick={decreseNumber}>-</button><button class="amount" onClick={handleAmountOf}>amount: {number}</button><button class="plus" onClick={increseNumber}>+</button><button class="remove" onClick={() => handleRemovePoster(index)}>remove poster</button>
        </div>
    );
}
